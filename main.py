...
# Define create_http_handler if not imported from fastmcp
def create_http_handler(mcp_instance):
    async def handler(request: Request):
        body = await request.body()
        headers = dict(request.headers)
        response = await mcp_instance.handle_http(body, headers)
        return HTMLResponse(content=response.body, status_code=response.status, headers=response.headers)
    return handler

mcp_handler = create_http_handler(mcp)
app.add_api_route("/", mcp_handler, methods=["POST"])
